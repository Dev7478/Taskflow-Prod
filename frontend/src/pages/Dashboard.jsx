...
export default function Dashboard() {
  ...
  const openUpdateModal = (ticket) => {
    ...
    setUpdateDueDate(ticket.dueDate);
  ...
  // Direct update without confirmation modal
  const handleUpdateSubmit = async (e) => {
    e.preventDefault();

    if (!updateSubject.trim() || !updateDescription.trim() || !updateDueDate) {
      showNotification("error", "Please fill all fields.");
      return;
    }

    if (!token || !updateTicketId) {
      showNotification("error", "Authentication required");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8082/api/tickets/details`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      const data = await res.json();
      // Check for both statusCode and status fields (backend might return either)
      if (res.ok && (data.statusCode === 200 || data.status === 200)) {
        showNotification("success", data.message || "Ticket updated successfully");
        closeUpdateModal();
        await fetchTickets(); // refresh tickets
      } else {
        const msg = data?.message || `Failed to update ticket (${res.status})`;
        showNotification("error", msg);
      }
    } catch (err) {
      console.error("Update error:", err);
      showNotification("error", "Network error while updating ticket");
    } finally {
  ...
  return (
    <div className="flex flex-col p-8 min-h-screen bg-gray-50">
                ...
                {tickets.map((ticket) => (
                  <tr key={ticket.id} className="hover:bg-gray-50 cursor-pointer transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-600">
                      <Link to={`/tickets/${ticket.id}/comments`} className="hover:underline">
                        {ticket.subject}
                      </Link>
                    </td>
      ...
      {/* Confirmation Modal for Delete Only */}
      <ConfirmationModal
      ...
      {isUpdateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black opacity-40" onClick={closeUpdateModal} />
            ...
            <form onSubmit={handleUpdateSubmit}>
              ...
              <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
                <button
                  type="button"
                  onClick={closeUpdateModal}
                  disabled={updateSubmitting || loading}
                  className="px-4 py-2 bg-gray-100 text-gray-700 rounded hover:bg-gray-200"
                >
                  Cancel
                </button>
